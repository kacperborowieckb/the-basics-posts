...
We will take a look at just a basics of React Hook Form. We won't focus on styling or more advanced use cases such as working with external UI libraries and using "Controller".

## Why React Hook Form?

React Hook Form is an amazing lightweight tool. It allows to handle form state and validate it very easily. It is also more performant than normal controlled forms. It doesn't re-render a component on every input change like with `useState` approach. With all of that, it is very easy to understand as well and that's just a main things. For more take a closer look at [React Hook Form documentation](https://react-hook-form.com/docs).
